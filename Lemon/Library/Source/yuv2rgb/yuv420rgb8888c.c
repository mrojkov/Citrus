...
enum
{
    YUVLIB_FLAGS         = 0x40080100 // buz fix: rename FLAGS to YUVLIB_FLAGS
};
...
#define FIXUP(Y)                 \
do {                             \
    int tmp = (Y) & YUVLIB_FLAGS;       \
    if (tmp != 0)                \
    {                            \
        tmp  -= tmp>>8;          \
        (Y)  |= tmp;             \
        tmp   = YUVLIB_FLAGS & ~(Y>>1); \
        (Y)  += tmp>>8;          \
    }                            \
} while (0 == 1)

// buz fix: write FF to alpha, and swap R and B channels
#define STORE(Y,DSTPTR)     \
do {                        \
    (DSTPTR) = 0xFF000000 | ((Y & 0xFF)<<16) | (0xFF00 & (Y>>14)) | ((0xFF0000 & (Y<<5))>>16);\
} while (0 == 1)
