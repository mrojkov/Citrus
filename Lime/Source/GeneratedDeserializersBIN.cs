...
namespace GeneratedDeserializersBIN
{
	public class BinaryDeserializerGen: BinaryDeserializerGenBase
	{
		...
		private static void Read_Lime__Model3DAttachmentParser__ModelAttachmentFormat(BinaryDeserializer d, ReaderClassDef def, object obj)
		{
			...
			if (2 == fd.OurIndex) {
				result.EntryAnimation = d.Reader.ReadString();
				if (result.EntryAnimation == "" && d.Reader.ReadBoolean()) result.EntryAnimation = null;
				fd = def.Fields[d.Reader.ReadInt16()];
			}
			if (3 == fd.OurIndex) {
				result.EntryMarker = d.Reader.ReadString();
				if (result.EntryMarker == "" && d.Reader.ReadBoolean()) result.EntryMarker = null;
				fd = def.Fields[d.Reader.ReadInt16()];
			}
			if (4 == fd.OurIndex) {
