	...
	public class BinaryDeserializerGen: BinaryDeserializerGenBase
	{
		...
		private static void Read_Lime__Camera3D(BinaryDeserializer d, ReaderClassDef def, object obj)
		{
			...
			if (8 == fd.OurIndex) {
				...
				if (tmp5 >= 0) {
					...
					while (--tmp5 >= 0) {
						var tmp6 = (global::Lime.Folder.Descriptor)dg.ReadObject<global::Lime.Folder.Descriptor>();
						result.Folders.Add(tmp6);
					}
				}
				fd = def.Fields[d.Reader.ReadInt16()];
			}
			...
			if (11 == fd.OurIndex) {
				...
				if (tmp7 >= 0) {
					while (--tmp7 >= 0) {
						var tmp8 = (global::Lime.Node)dg.ReadObject<global::Lime.Node>();
						result.Nodes.Add(tmp8);
					}
				}
				fd = def.Fields[d.Reader.ReadInt16()];
			}
		...
		private static void Read_Lime__Mesh3D(BinaryDeserializer d, ReaderClassDef def, object obj)
		{
			...
			if (15 == fd.OurIndex) {
				...
				if (tmp9 >= 0) {
					while (--tmp9 >= 0) {
						var tmp10 = (global::Lime.Submesh3D)dg.ReadObject<global::Lime.Submesh3D>();
						result.Submeshes.Add(tmp10);
					}
				}
				fd = def.Fields[d.Reader.ReadInt16()];
			}
		...
		private static void Read_Lime__Spline3D(BinaryDeserializer d, ReaderClassDef def, object obj)
		{
			...
			if (10 == fd.OurIndex) {
				...
				if (tmp9 >= 0) {
					...
					while (--tmp9 >= 0) {
						var tmp10 = (global::Lime.Spline3D.Point)dg.ReadObject<global::Lime.Spline3D.Point>();
						result.Points.Add(tmp10);
					}
				}
				fd = def.Fields[d.Reader.ReadInt16()];
			}
