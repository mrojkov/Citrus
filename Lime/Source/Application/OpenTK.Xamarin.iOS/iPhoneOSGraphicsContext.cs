...
namespace Lime.Xamarin {
    ...
    public class iPhoneOSGraphicsContext : IGraphicsContext, IGraphicsContextInternal {
        ...
        {
            // ignore mode, window
			iPhoneOSGraphicsContext shared = null;
			var internalSharedContext = sharedContext as OpenTK.Graphics.IGraphicsContextInternal;
			if (internalSharedContext != null) { 
				shared = internalSharedContext.Implementation as iPhoneOSGraphicsContext;
			}
            EAGLRenderingAPI version = 0;
            if (major == 1 && minor == 1)
                version = EAGLRenderingAPI.OpenGLES1;
