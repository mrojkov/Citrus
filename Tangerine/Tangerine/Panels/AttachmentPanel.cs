			...
			using (var cacheBundle = new PackedAssetBundle(Orange.The.Workspace.TangerineCacheBundle)) {
				...
				if (metaCached) {
					using (var assetStream = cacheBundle.OpenFile(metaPath)) {
						meta = TangerinePersistence.Instance.Value.ReadObject<Model3DAttachmentMeta>(metaPath, assetStream);
					}
				}
			}
			if (!metaCached) {
			...
			using (var cacheBundle = new PackedAssetBundle(Orange.The.Workspace.TangerineCacheBundle)) {
				...
				if (attachmentCached) {
					using (var assetStream = cacheBundle.OpenFile(path)){
						var attachmentFormat = TangerinePersistence.Instance.Value.ReadObject<Model3DAttachmentParser.ModelAttachmentFormat>(path, assetStream);
